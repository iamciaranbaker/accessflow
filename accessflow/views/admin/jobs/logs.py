...
from accessflow.filters import format_time, format_datetime
import json
...
class AdminJobLogsView(View):
    ...
    def dispatch_request(self):
        ...
        # Only fetch the logs from a certain point - used for realtime job log viewing
        ...
        if job_run_id:
            ...
            job_run = JobRun.query.filter(JobRun.id == job_run_id, JobRun.job_id == job_id).order_by(JobRun.started_at.desc()).first()
        else:
            # Fetch the latest job run for the given job ID
            job_run = JobRun.query.filter(JobRun.job_id == job_id).order_by(JobRun.started_at.desc()).first()
        ...
        if offset is not None:
            logs = JobLog.query.filter(JobLog.job_run_id == job_run.id).order_by(JobLog.id).offset(offset).all()
            ...
            if job_run.status != JobRunStatus.RUNNING:
                if job_run.started_at and job_run.ended_at:
                    response["duration"] = format_time(job_run.ended_at - job_run.started_at)
                response["finished_at"] = format_datetime(job_run.ended_at)
