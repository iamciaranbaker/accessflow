...
class Job(db.Model):
    ...
    run_interval = db.Column(db.BigInteger, nullable = False)
    ...
    def seed_all():
        jobs = [
            Job("check_gl_pat_expiries", "accessflow.jobs.check_gl_pat_expiries", "CheckGLPATExpiries", 60)
        ]

        for job in jobs:
            existing_job = Job.query.filter_by(name = job.name).first()
            if existing_job:
    ...
    @staticmethod
    def run_all():
        jobs = Job.query.filter(Job.last_run_at + Job.run_interval < db.func.now()).all()
